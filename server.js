...
// Simple route handlers for HTML files
app.get('/', (req, res) => { 
...
function getShareEmailTemplate(senderName, senderEmail, recipientName, taskCount, shareType, expiresAt, shareLink, message = null) {
    return `
        ...
        <style>
            ...
            .button { display: inline-block; background: linear-gradient(135deg, #667eea, #764ba2); color: #fff; padding: 12px 30px; text-decoration: none; border-radius: 8px; margin: 20px 0; font-weight: 600; }
            .warning { background: #fed7d7; border-left-color: #f56565; padding: 12px; border-radius: 8px; margin: 15px 0; }
            .footer { background: #f7fafc; padding: 20px; text-align: center; font-size: 12px; color: #718096; }
            hr { border: none; border-top: 1px solid #e2e8f0; margin: 20px 0; }
        </style>
        </head>
        ...
        <div class="container">
            <div class="header">
                <h1>⚡ TaskWeaver</h1>
                <p>Schedule Sharing Notification</p>
            </div>
            <div class="content">
                <h2>Hello ${recipientName}! 👋</h2>
                <p><strong>${senderName}</strong> (${senderEmail}) has shared their schedule with you.</p>
                ${message ? `<div class="warning"><strong>📝 Personal Message:</strong><br>${message}</div>` : ''}
                <div class="info-box">
                    <strong>📅 Schedule Details:</strong><br>
                    • Total Tasks: ${taskCount}<br>
                    • Share Type: ${shareType === 'view' ? 'View Only' : 'Full Access'}<br>
                    • Expires: ${moment(expiresAt).format('MMMM Do YYYY, h:mm a')}<br>
                    • Time Remaining: ${moment(expiresAt).fromNow()}
                </div>
                <div style="text-align: center;">
                    <a href="${shareLink}" class="button">📅 View Schedule Online</a>
                </div>
                <hr>
                <small>🔒 This link expires in 24 hours for security. All access is logged and monitored.<br>
                If you didn't expect this email, you can safely ignore it.</small>
            </div>
            <div class="footer">
                <p>© 2025 TaskWeaver. All rights reserved.</p>
...
// ============ SHARING AND EXPORT WITH ENHANCED FEATURES ============
app.post('/api/share-schedule', requireAuth, async (req, res) => {
    ...
    try {
        ...
        const shareLink = `${process.env.BASE_URL || `https://${req.get('host')}`}/shared-schedule.html?token=${shareToken}`;
...
// Get all shared schedules for the current user
app.get('/api/my-shared-schedules', requireAuth, async (req, res) => {
    try {
        ...
        const formattedSchedules = result.rows.map(schedule => ({
            ...
            accessCount: schedule.access_count,
            lastAccessed: formatTimestampForResponse(schedule.last_accessed),
            taskCount: parseInt(schedule.task_count),
            status: schedule.status,
            shareLink: `${process.env.BASE_URL || `https://${req.get('host')}`}/shared-schedule.html?token=${schedule.share_token}`
        }));
...
// Extend expiration of a shared schedule
